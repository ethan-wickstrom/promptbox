...
import { closeDb, createServer } from "./server.ts"
...
describe("server api", () => {
  ...
  it("retrieves a prompt by id", async () => {
    ...
    const getRes = await fetch(new URL(`/api/prompts/${created.id}`, server.url))
  ...
  it("updates a prompt", async () => {
    ...
    const created = await createRes.json()
    const updateRes = await fetch(new URL(`/api/prompts/${created.id}`, server.url), {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  it("deletes a prompt", async () => {
    ...
    const created = await createRes.json()
    const deleteRes = await fetch(new URL(`/api/prompts/${created.id}`, server.url), { method: "DELETE" })
    expect(deleteRes.status).toBe(204)
    const listRes = await fetch(new URL("/api/prompts", server.url))
    const prompts = await listRes.json()
    expect(prompts.find((p: { id: string }) => p.id === created.id)).toBe(undefined)
    const getRes = await fetch(new URL(`/api/prompts/${created.id}`, server.url))
